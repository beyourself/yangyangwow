    ...
    sLog->outString("Loading Player Create Spell Data...");
    {
        ...
        else
        {
            ...
            do
            {
                ...
                {
					sLog->outErrorDb("Wrong race mask %u in `playercreateinfo_spell` table, ignoring.", current_race);
                    continue;
                }
				...
				if (current_class >= MAX_CLASSES)
                {
					sLog->outErrorDb("Wrong class mask %u in `playercreateinfo_spell` table, ignoring.", current_class);
                    continue;
                }
				...
				if (!current_race || !current_class)
                {
					...
					for (uint32 r = min_race; r < max_race; ++r)
					 ...
					 {
						 sLog->outErrorDb("Wrong race: %u, class: %u combination in `playercreateinfo_spell` table, ignoring.", current_race, current_class);
