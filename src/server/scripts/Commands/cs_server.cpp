...
public:
    ...
    static bool HandleServerInfoCommand(ChatHandler* handler, char const* /*args*/)
    {
		...
		handler->PSendSysMessage("wow3.3.5 Realm, blizard 10x.", realmName.c_str(), _REVISION);
		handler->PSendSysMessage("welcome 3.3.5 10x Blizzard");
		if (!queuedSessionCount)
			handler->PSendSysMessage("在线玩家人数: %u. ", activeSessionCount, playerCount);
		else
			handler->PSendSysMessage("online: %u. . Queue: %u.", activeSessionCount, playerCount, queuedSessionCount);
		//handler->PSendSysMessage("Connection peak: %u.", connPeak);
        handler->PSendSysMessage(LANG_UPTIME, uptime.c_str());
	    handler->PSendSysMessage("服务器更新时间 diff: %ums, 平均: %ums.", updateTime, avgUpdateTime);
